...
export const generatePayrollBatchPDF = (payrollData) => {
  ...
  const startDateStr = businessDateKey(new Date(payrollData.batchHeader.payrollPeriod.startDate));
  const endDateStr = businessDateKey(new Date(payrollData.batchHeader.payrollPeriod.endDate));
  const fileName = `Payroll_Register_${startDateStr}_to_${endDateStr}.pdf`;
  
  pdf.save(fileName);
};

export const generateRevenuePDF = (revenueData, selectedPeriod, reportDateKey = null) => {
  ...
  pdf.text(`Generated on: ${generatedOn}`, pageWidth / 2, yPosition, { align: 'center' });
  yPosition += 5;

  if (reportDateKey) {
    pdf.text(`Report date: ${reportDateKey}`, pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 10;
  } else {
    yPosition += 10;
  }
  ...
  // Generate filename and save
  const dateForFile = reportDateKey || businessDateKey(new Date());
  const fileName = `Revenue_Report_${selectedPeriod.charAt(0).toUpperCase() + selectedPeriod.slice(1)}_${dateForFile}.pdf`;
  pdf.save(fileName);
};
...
export const generateYearlyRevenuePDF = (reportData, periodLabel) => {
  ...
  pdf.setFontSize(16);
  pdf.text('Yearly Financial Report', pageWidth / 2, yPosition, { align: 'center' });
  yPosition += 8;
  ...
  pdf.setFontSize(9);
  pdf.text('Figures are based on accrual accounting (incurred revenue and expenses).', pageWidth / 2, yPosition, { align: 'center' });
  yPosition += 8;
  ...
  const expenseEntries = Object.entries(reportData?.expenseByCategory || {})
  ...
  // Generate filename and save
  const fileName = `Yearly_Financial_Report_${periodLabel.replace(/\s+/g, '_')}_${businessDateKey(new Date())}.pdf`;
  pdf.save(fileName);
};
...
export const generatePayslipPDF = (payslipData) => {
  ...
  const periodDate = businessDateKey(new Date(payslipData.payrollPeriod));
 ...
 * Proper tabular export with header, filters, totals, and pagination.
 * @param {Object} params
 * @param {Array} params.expenses - List of expense documents
 * @param {Object} params.filters - Applied filters (dateRange/category/status/search)
 * @param {string} params.periodLabel - Human readable period label
 * @param {string} [params.generatedBy] - Username
 */
export const generateExpenseReportPDF = ({ expenses = [], filters = {}, periodLabel = '', generatedBy = '' }) => {
  const pdf = new jsPDF({
    orientation: 'portrait',
    unit: 'mm',
    format: 'a4',
  ...
  const addFooter = () => {
    ...
    const footer = `Generated by Ring & Wing POS System • Page ${pageNum}`;
  ...
  pdf.setFont('helvetica', 'bold');
  pdf.setFontSize(16);
  pdf.text('Ring & Wing Restaurant', pageWidth / 2, y, { align: 'center' });
  y += 7;

  pdf.setFontSize(12);
  pdf.text('Expense Report', pageWidth / 2, y, { align: 'center' });
  y += 6;
  ...
  const generatedByText = generatedBy ? ` • By: ${generatedBy}` : '';
  pdf.text(`Generated on: ${generatedOn}${generatedByText}`, pageWidth / 2, y, { align: 'center' });
  y += 8;
  ...
  if (filters?.paymentStatus && filters.paymentStatus !== 'All') filterParts.push(`Status: ${filters.paymentStatus}`);
  if (filters?.search) filterParts.push(`Search: ${filters.search}`);
  if (filterParts.length > 0) {
    pdf.setFontSize(9);
    pdf.setFont('helvetica', 'bold');
    pdf.text('Filters:', margin, y);
    pdf.setFont('helvetica', 'normal');
    const filterText = filterParts.join(' • ');
    const wrapped = pdf.splitTextToSize(filterText, usableWidth - 14);
  ...
  const paidAmount = expenses
    .filter((e) => e?.disbursed || e?.status === 'paid')
    .reduce((sum, e) => sum + Number(e?.amount || 0), 0);
  const pendingAmount = totalAmount - paidAmount;

  pdf.setFont('helvetica', 'bold');
  pdf.setFontSize(10);
  pdf.text(`Total Records: ${expenses.length}`, margin, y);
  y += 5;

  pdf.setFontSize(9);
  pdf.setFont('helvetica', 'normal');
  pdf.text(`Total Amount: ${formatCurrency(totalAmount)}`, margin, y);
  y += 4;
  pdf.text(`Paid Amount: ${formatCurrency(paidAmount)}`, margin, y);
  y += 4;
  pdf.text(`Pending Amount: ${formatCurrency(pendingAmount)}`, margin, y);
  y += 7;
  ...
  const cols = [
    ...
    { key: 'paymentMethod', label: 'Method', width: 22 },
    { key: 'amount', label: 'Amount', width: 22 },
    { key: 'status', label: 'Status', width: 18 },
    { key: 'description', label: 'Description', width: usableWidth - (22 + 26 + 22 + 22 + 18) }
  ];
  ...
  sorted.forEach((e) => {
    ...
    const descriptionLines = pdf.splitTextToSize(row.description, cols.find(c => c.key === 'description').width - 2);
    const rowHeight = Math.max(6, descriptionLines.length * 3.6 + 2);
    ...
    x = margin + 1.5;
    pdf.text(row.date, x, y + 4);
    x += cols[0].width;

    pdf.text(row.category.substring(0, 16), x, y + 4);
    x += cols[1].width;

    pdf.text(row.paymentMethod.substring(0, 14), x, y + 4);
    x += cols[2].width;

    pdf.text(row.amount, x, y + 4);
    x += cols[3].width;

    pdf.text(row.status.replace('_', ' ').toUpperCase().substring(0, 10), x, y + 4);
    x += cols[4].width;

    pdf.text(descriptionLines, x, y + 4);
  ...
  const safePeriod = (periodLabel || 'All').replace(/[^a-zA-Z0-9_-]+/g, '_');
  const fileName = `Expense_Report_${safePeriod}_${businessDateKey(new Date())}.pdf`;
  pdf.save(fileName);
};
