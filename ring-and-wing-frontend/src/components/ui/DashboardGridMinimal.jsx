...
export const DashboardGridMinimal = ({
  ...
  return (
    <div className={`${className} flex flex-col gap-4`}>
      {/* KPI Row with Bigger Fonts */}
      <div className="grid grid-cols-4 gap-3">
        <Card className="!p-4 flex flex-row items-center">
          <div className="w-10 h-10 rounded-full flex items-center justify-center mr-3"
               style={{ backgroundColor: theme.colors.primary + '15' }}>
            <PesoIconClean width={20} height={20} style={{ color: theme.colors.primary }} />
          </div>
          <div>
        ...
        <Card className="!p-4 flex flex-row items-center">
          <div className="w-10 h-10 rounded-full flex items-center justify-center mr-3"
            ...
            <FiShoppingBag className="w-5 h-5" style={{ color: theme.colors.accent }} />
          </div>
          <div>
        ...
        <Card className="!p-4 flex flex-row items-center">
          <div className="w-10 h-10 rounded-full flex items-center justify-center mr-3"
            ...
            <FiTrendingUp className="w-5 h-5" style={{ color: theme.colors.secondary }} />
          </div>
          <div>
            <div className="text-sm text-gray-500">Avg</div>
            <div className="font-bold text-lg" style={{ color: theme.colors.secondary }}>
        ...
        <Card className="!p-4 flex flex-row items-center">
          <div className="w-10 h-10 rounded-full flex items-center justify-center mr-3"
            ...
            <FiUsers className="w-5 h-5" style={{ color: theme.colors.muted }} />
          </div>
          <div>
            <div className="text-sm text-gray-500">Staff</div>
            <div className="font-bold text-lg" style={{ color: theme.colors.muted }}>
              {stats.activeStaff || 0}
            </div>
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <div key={item.id || index} className="flex flex-col">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-sm capitalize truncate max-w-[90px]" title={item.name}>{item.name}</span>
                    </div>
        ...
        <Card className="!p-0 overflow-hidden">
          <div className="px-3 py-2 border-b flex justify-between items-center" 
               style={{ borderColor: theme.colors.muted + '15' }}>
            <div className="text-sm font-semibold flex items-center gap-1" style={{ color: theme.colors.primary }}>
              <PesoIconSimple width={14} height={14} /> Revenue Overview
            </div>
            <Link 
              to="/revenue-reports" 
              className="flex items-center text-sm font-medium"
          ...
          <div className="p-3">
            ...
            <div className="mb-3">
              ...
              {revenueData.length > 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={revenueData.slice(-7)} barSize={8}>
                      ...
                      <Tooltip
                        ...
                        contentStyle={{ fontSize: '12px', backgroundColor: theme.colors.background }}
                ...
                <div className="h-16 flex items-center justify-center bg-gray-50 rounded">
                  <span className="text-sm" style={{ color: theme.colors.muted }}>No revenue data available</span>
                </div>
              )}
            </div>
            ...
            <div className="mb-3">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm" style={{ color: theme.colors.muted }}>Profit Margin</span>
                <span 
              ...
              <div className="flex justify-between mt-1">
                <span className="text-sm" style={{ color: theme.colors.muted }}>0%</span>
                <span className="text-sm" style={{ color: theme.colors.muted }}>Target: 25%</span>
                <span className="text-sm" style={{ color: theme.colors.muted }}>50%</span>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm" style={{ color: theme.colors.muted }}>Cash Flow</span>
                <span 
                  className="text-base font-bold"
                  style={{ 
          ...
          <div className="p-3">
            {monthlyExpenses.length > 0 ? (
              ...
              <div className="h-32 flex items-center justify-center bg-gray-50 rounded">
                <span className="text-sm" style={{ color: theme.colors.muted }}>No expense data</span>
              </div>
            )}
          </div>
        </Card>
          ...
          <div className="p-3">
            <div className="h-24">
              {revenueData && revenueData.length > 0 ? (
                ...
                <div className="text-sm text-center py-4" style={{ color: theme.colors.muted }}>
