...
export const PrintableYearlyReport = forwardRef(({
  ...
  return (
    ...
    >
      ...
      {/* Summary Metrics */}
      <div className="mb-8">
        <h3 className="text-lg font-semibold mb-4" style={{ color: colors.primary }}>Financial Summary</h3>
        <div className="grid grid-cols-4 gap-4">
          <div className="border rounded-lg p-4 text-center" style={{ borderColor: colors.muted + '40' }}>
            ...
            <div className="text-xl font-bold" style={{ color: colors.success }}>{formatCurrency(summary.totalRevenue)}</div>
          </div>
          <div className="border rounded-lg p-4 text-center" style={{ borderColor: colors.muted + '40' }}>
            <div className="text-sm mb-1" style={{ color: colors.muted }}>Total Expenses</div>
            <div className="text-xl font-bold" style={{ color: colors.error }}>{formatCurrency(summary.totalExpenses)}</div>
          </div>
          <div className="border rounded-lg p-4 text-center" style={{ borderColor: colors.muted + '40' }}>
            <div className="text-sm mb-1" style={{ color: colors.muted }}>Net Revenue</div>
            <div className="text-xl font-bold" style={{ color: summary.netRevenue >= 0 ? colors.success : colors.error }}>{formatCurrency(summary.netRevenue)}</div>
          </div>
          <div className="border rounded-lg p-4 text-center" style={{ borderColor: colors.muted + '40' }}>
            <div className="text-sm mb-1" style={{ color: colors.muted }}>Profit Margin</div>
            <div className="text-xl font-bold" style={{ color: colors.primary }}>{summary.profitMargin}%</div>
          </div>
        </div>
      </div>
      ...
      {chartData.length > 0 && (
        <div className="mb-8">
          <h3 className="text-lg font-semibold mb-4" style={{ color: colors.primary }}>{viewMode === 'monthly' ? 'Monthly' : 'Quarterly'} Revenue vs Expenses</h3>
          <div className="h-64 border rounded-lg p-4" style={{ borderColor: colors.muted + '40', backgroundColor: 'white' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <XAxis dataKey="name" tick={{ fontSize: 10, fill: '#000000' }} axisLine={{ stroke: '#000000' }} tickLine={{ stroke: '#000000' }} />
                <YAxis tickFormatter={(v) => `₱${(v/1000).toFixed(0)}k`} tick={{ fontSize: 10, fill: '#000000' }} axisLine={{ stroke: '#000000' }} tickLine={{ stroke: '#000000' }} />
      ...
      {monthlyBreakdown.length > 0 && (
        <div className="mb-8">
          <h3 className="text-lg font-semibold mb-4" style={{ color: colors.primary }}>Monthly Breakdown</h3>
          <table className="w-full border rounded-lg" style={{ borderColor: colors.muted + '40' }}>
            ...
            <tbody>
              {monthlyBreakdown.map((row, idx) => (
                <tr key={row?.month || idx} className="border-t" style={{ borderColor: colors.muted + '20' }}>
                  ...
                  <td className="p-3 text-right text-sm">{row?.orderCount || 0}</td>
                </tr>
              ))}
            </tbody>
            <tfoot style={{ backgroundColor: colors.activeBg }}>
              <tr className="border-t-2 font-bold" style={{ borderColor: colors.accent }}>
                ...
                <td className="p-3 text-right text-sm" style={{ color: colors.success }}>{formatCurrency(summary.totalRevenue)}</td>
                <td className="p-3 text-right text-sm" style={{ color: colors.error }}>{formatCurrency(summary.totalExpenses)}</td>
                <td className="p-3 text-right text-sm" style={{ color: summary.netRevenue >= 0 ? colors.success : colors.error }}>{formatCurrency(summary.netRevenue)}</td>
                <td className="p-3 text-right text-sm">{summary.totalOrders}</td>
              </tr>
            </tfoot>
          </table>
        </div>
      ...
      {quarterlyBreakdown.length > 0 && (
        <div className="mb-8">
          <h3 className="text-lg font-semibold mb-4" style={{ color: colors.primary }}>Quarterly Breakdown</h3>
          <table className="w-full border rounded-lg" style={{ borderColor: colors.muted + '40' }}>
            <thead style={{ backgroundColor: colors.activeBg }}>
              <tr>
            ...
            <tbody>
              {quarterlyBreakdown.map((row, idx) => (
                <tr key={row?.quarter || idx} className="border-t" style={{ borderColor: colors.muted + '20' }}>
          ...
          <table className="w-full border rounded-lg" style={{ borderColor: colors.muted + '40' }}>
            ...
            <tbody>
              {topItems.slice(0, 10).map((item, index) => (
                <tr key={item?.name || index} className="border-t" style={{ borderColor: colors.muted + '20' }}>
      ...
      <div className="border-t pt-6 mt-8 text-center text-sm" style={{ color: colors.muted }}>
        ...
        <div className="mt-2">Ring & Wing Restaurant</div>
        <div>Generated on {formatDate(reportDate)}</div>
      </div>
    </div>
  );
});
